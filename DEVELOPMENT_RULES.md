...
| Preview / listing fields / what the extension fills | `src/lib/listingData.ts` header + `tests/listingData.test.ts`; `PROJECT_KNOWLEDGE.md` Bug 7; `public/extension/content.js` field signatures (`namaProduk`, `deskripsi`, `penawaran`, …) |
...
| Tema Persona / pemandu Mitsuru / gerak bertema | `THEME_PERSONA.md` (kontrak desain: token, tipografi, bentuk, bahasa gerak, pemetaan layar); `PROJECT_KNOWLEDGE.md` entri "Persona Theme"; `src/theme/`, `src/guide/`, `src/ui/` header files once they exist |
...
- `campaign.dongkrakListingData` is a VIEW built by `src/lib/listingData.ts` `buildListingData()`, never hand-assembled. Any code that changes `generatedContent`, `seoStrategy` or `businessData` and stores the campaign rebuilds the listing through the builder, or the Preview and the autofill show yesterday's text (Bug 7).
...
- The DongkrakUsaha description field is a plain textarea (the filler's CKEditor path is a fallback): no `maxlength`, so its limit is only provable by saving and re-measuring (extension "Ukur Field Halaman Ini"). Only the ~25 meta inputs declare a maxlength (165). Do not cite a limit that has not been measured that way.
- Reloading or updating the unpacked extension kills the bridge in every open app tab; only the background can re-inject it, and a re-injected bridge must be SENT the state (it missed the broadcast). "Popup sees fields, app shows 0" is always this -- see PROJECT_KNOWLEDGE Bug 6 before touching the handshake code. After any extension change tell the owner: reload the extension, then popup -> Refresh Status (or reload the app page).
...
- Themes: the standard UI is the default and must stay pixel-identical; the Persona theme lives ONLY under `[data-theme="persona"]` in `src/theme/persona.css` (Tailwind v4 variable remap + hook classes `du-card`, `du-btn`, `du-badge`, `du-label`, `du-nav-item`, `du-cutout`, `du-dark`). Hook classes carry no styles in the standard theme. A component that is dark by design gets `du-dark` on its wrapper so the slate inversion skips it. Prove "default unchanged" with the computed-style fingerprint script (PROJECT_KNOWLEDGE "Persona Theme, Stage 1") before committing theme work. `CutoutText` renders bare text in the standard theme -- never wrap it in an extra element there.
- Every new interactive element the operator can point at gets `data-guide="<area>.<id>"` AND an entry in `src/guide/guideRegistry.ts` (fungsi + fakta from PROJECT_KNOWLEDGE). Canvas nodes are covered by `data-node`. `npm test` fails when a nav tab or canvas node has no entry; put the attribute on the button, not on its icon. The guide (`src/guide/GuideCompanion.tsx`) renders nothing in the standard theme and below 1024 px; it must never cover the bottom nav, the status widget or the canvas controls -- under Persona the canvas controls are mirrored to the left (`du-canvas-controls`).
- The theme choice is a per-viewer preference (`localStorage du-theme`, applied pre-paint by the inline script in `index.html`); never a server setting.
