...
The length rule is whatever `src/lib/lengthRule.ts` parses from the operator's instruction (kata / kalimat / karakter; default 500-1000 kata). Ceilings (1.500 kata, 120 kalimat, 10.000 karakter) exist because of the 30/60 s call timeouts, not the models' token limits; raise them only together with the timeouts. Measured this day: flash-lite undershoots a per-section budget by ~35 % on a 950-1000 word request (596→728 over three passes) while landing 54 sentences for "40-60 kalimat" first try; Flash lands inside word budgets. The content stage now produces a 500-1000 word article (roughly 1.2-2k output tokens, well inside every candidate's limit and the 30 s hedged timeout on Flash: 10 s measured). Models do not count their own words -- flash-lite lands ~15 % under a per-section budget, Flash lands inside it -- so `server.ts` counts and, on a miss, spends ONE extra content call with the measured number. Budget one extra call per content stage in storm mode (flash-lite fallback) and none in calm mode.
